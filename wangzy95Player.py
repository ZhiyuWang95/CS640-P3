...
def evaluator(board, lastPlay, maxx):  #figure out score for move
	total = 0
	if Loserfunction(board, lastPlay):  #if move loses return neg infin
		if not maxx:
...
def minimaxAB(board, lastPlay, depth, maxx, alpha, beta):  #minimax & a-b pruning
	if lastPlay[0] == "null":
		return (0, [3, size, 1, 1])
	if depth == 0 or Loserfunction(board, lastPlay):
		return evaluator(board, lastPlay, maxx)
	else:
